...
- Eight towers: repulsor, mortar, autocannon, cryo emitter, Tesla coil, rocket pod, railgun, and incinerator. Three normal enemy kinds: shambler, runner, brute. Each level has ten escalating procedural waves, with the Bulldozer on wave ten. Clearing a level resets defenses, local research, Metal, and base integrity; permanent Command XP upgrades remain.
- Space pauses/resumes; H toggles the pressure overlay. Restart clears the current attackers and begins the same wave again while keeping defenses in place. Save creates a full defense checkpoint between waves in this browser: map, player walls, wire condition, towers, research, economy, and Zombie Flow. Load restores that checkpoint. Automatic between-wave saves resume on reload and never overwrite the manual checkpoint. Existing full autosaves still resume; use Save once to create a manual checkpoint with the new format. Switching modes or resetting starts a fresh run; it does not overwrite a saved defense.
