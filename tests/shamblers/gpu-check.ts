...
export async function checkAnimation(device:GPUDevice){
  ...
  try{
    ...
    p[0]+=.04;a=await step(.02);assert(a[7]===1&&a[5]!==phase,'Moving shambler failed to walk');
    ...
    p[11]=1;p[7]=30;p[15]=2;p[2]=0;p[3]=2;a=await step(.02);assert(Math.abs(a[4]-Math.PI/2)<.001&&a[6]===0,'Recycled slot inherited old facing or stagger');
