...
try{
 ...
 assert(counters[0]===4096,`Burst lost kills: ${counters[0]} / ${count}; live ${counters[4]}; attributed ${counters[16]}`);assert(counters[16]===4096,'Burst lost turret attribution');assert(counters[3]===245&&counters[15]===12288,'Incorrect base payout or lost bounty remainder');
 ...
 assert(run.model.metal-initialMetal===306,'Salvage payout is incorrect');assert(tower.veterancyXp===4096&&tower.kills===4096,'XP missing or awarded twice');
 const error=await device.popErrorScope();if(error)throw Error(error.message);
 status.textContent='PASS: 4,096 simultaneous kills → 245 base Metal + 61 salvage bonus; 4,096 turret XP. No duplicate rewards over 60 simulation ticks or repeated readbacks.';
