...
export function createUI(
  ...
  const renderTowerStats = (s: UIState) => {
    ...
    stats.innerHTML = `<div><span>KILLS</span><b>${(t.kills ?? 0).toLocaleString()}</b></div><div><span>VETERANCY</span><b>RANK ${rank} / ${MAX_VETERANCY}</b><small>${next}</small></div><div><span>OUTPUT${preview ? " · NEXT UPGRADE" : ""}</span><b>${formatPressure(d.peakPressureKpa)} PEAK ${preview ? statDelta(preview.peakPressureKpa-d.peakPressureKpa,0) : ""} · ${d.range.toFixed(0)} RANGE ${preview ? statDelta(preview.range - d.range, 0) : ""}</b><small>${d.force.toFixed(0)} IMPULSE ${preview ? statDelta(preview.force - d.force, 0) : ""} · ${(1 / d.cooldown).toFixed(1)} PULSES/S ${preview ? statDelta(1 / preview.cooldown - 1 / d.cooldown, 1) : ""} · ${d.radius.toFixed(1)} RADIUS ${preview ? statDelta(preview.radius - d.radius, 1) : ""}</small></div><div><span>MODIFIERS</span><small>${mods.join(" · ")}</small></div>`;
  };
