...
export function createUI(
  ...
  return {
    ...
    update(s: UIState) {
      ...
      const boss = bossStatus(s);
      $("#boss-status").hidden = !boss;
      if (boss) {
        $("#boss-health-text").textContent = `${Math.ceil(boss.health)} / ${Math.ceil(boss.maxHealth)} HP`;
        $<HTMLProgressElement>("#boss-health").value = boss.percent;
        // Announce phase transitions, not every telemetry refresh.
        if ($("#boss-phase").textContent !== boss.label) $("#boss-phase").textContent = boss.label;
        $("#boss-hint").textContent = boss.hint;
      }
      $<HTMLInputElement>("#difficulty").value = String(s.difficulty);
      $("#difficulty-value").textContent = `${s.difficulty}×`;
      const preview = previewNextWave(s);
      $("#wave-preview").hidden = !preview;
      if (preview) {
