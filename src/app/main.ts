...
/* Recycle hover branch variant is superseded here by the placement-preview wall model. */
 ...
 const restoreSession=()=>{try{const saved=JSON.parse(localStorage.getItem(AUTOSAVE_KEY)??'null') as {runState?:string;map?:WorldMap;spawnBaseline?:Rect;builtWalls?:(Rect & {health?:number;maxHealth?:number})[];builtWires?:(Rect & {health:number;maxHealth:number;breached:boolean})[];difficulty?:number}|null;if(!saved?.runState||!saved.map)return false;const capacity=wallCapacity(0);builtWalls=(saved.builtWalls??[]).filter(w=>Number.isFinite(w.x)&&Number.isFinite(w.y)).map(w=>({...w,health:Number.isFinite(w.health)?Math.max(0,w.health!):capacity,maxHealth:Number.isFinite(w.maxHealth)?Math.max(1,w.maxHealth!):capacity}));builtWires=(saved.builtWires??[]).filter(w=>Number.isFinite(w.x)&&Number.isFinite(w.y)&&Number.isFinite(w.health));const dynamic=[...builtWalls,...builtWires];const same=(a:Rect,b:Rect)=>a.x===b.x&&a.y===b.y&&a.width===b.width&&a.height===b.height;map={...saved.map,obstacles:[...saved.map.obstacles.filter(obstacle=>!dynamic.some(segment=>same(obstacle,segment))),...builtWalls,...builtWires.filter(wire=>!wire.breached)]};spawnBaseline=saved.spawnBaseline??saved.map.spawn;state.difficulty=run.setSpawnMultiplier(saved.difficulty??1);resizeSpawn();navigation=buildNavigation(map);run.setMap(map);run.setBuildMounts(builtWalls);return run.load(saved.runState).ok;}catch{return false;}};
 const newGame=()=>{
   try{localStorage.removeItem(AUTOSAVE_KEY);localStorage.removeItem('pressure-front.customlevel.v1');}catch{/* Local persistence is optional. */}
   run.clearSave();progression.reset();editor.resetToDefault();state.mode='game';state.difficulty=1;map=DEFAULT_MAP;spawnBaseline=DEFAULT_MAP.spawn;builtWalls=[];builtWires=[];navigation=buildNavigation(map);run.setMap(map);run.setBuildMounts([]);resetWorld();lastAutosave=performance.now();saveSession();state.message='New game started. All local progress and defenses were cleared.';
 };
 ...
 const placeWall=(wall:Rect)=>{const issue=defensePlacementIssue(map,run.model.towers,wall,run.model.metal,WALL_COST);if(issue){state.message=issue;return;}const result=run.spendMetal(WALL_COST);if(!result.ok){state.message=result.reason??'Could not build wall.';return;}const level=run.model.commandUpgrades.filter(id=>/^wall-engineering-\d+$/.test(id)).length,capacity=wallCapacity(level),placed={...wall,health:capacity,maxHealth:capacity};builtWalls.push(placed);map={...map,obstacles:[...map.obstacles,placed]};navigation=buildNavigation(map);run.setMap(map);run.setBuildMounts(builtWalls);state.message=`Metal wall installed. Towers can mount at its center; right-drag recovers ${WALL_REFUND} Metal.`;};
 const removeWall=(point:Vec2)=>{const index=builtWalls.findIndex(w=>point.x>=w.x&&point.x<w.x+w.width&&point.y>=w.y&&point.y<w.y+w.height);if(index<0)return;const [wall]=builtWalls.splice(index,1);map={...map,obstacles:map.obstacles.filter(existing=>existing!==wall)};navigation=buildNavigation(map);run.setMap(map);run.setBuildMounts(builtWalls);run.refundMetal(WALL_REFUND);state.message=`Metal wall recovered for ${WALL_REFUND} Metal.`;};
 const placeWire=(wire:Rect)=>{const issue=defensePlacementIssue(map,run.model.towers,wire,run.model.metal,WIRE_COST);if(issue){state.message=issue;return;}const result=run.spendMetal(WIRE_COST);if(!result.ok){state.message=result.reason??'Could not place wire.';return;}const stats=barbedWireStats(run.model.commandUpgrades),placed={...wire,health:stats.durability,maxHealth:stats.durability,breached:false};builtWires.push(placed);map={...map,obstacles:[...map.obstacles,placed]};navigation=buildNavigation(map);run.setMap(map);state.message='Barbed wire installed. It cuts and slows enemies until local pressure forces a breach.';};
 const removeWire=(point:Vec2)=>{const index=builtWires.findIndex(w=>point.x>=w.x&&point.x<w.x+w.width&&point.y>=w.y&&point.y<w.y+w.height);if(index<0)return;const [wire]=builtWires.splice(index,1);map={...map,obstacles:map.obstacles.filter(existing=>existing!==wire)};navigation=buildNavigation(map);run.setMap(map);run.refundMetal(WIRE_REFUND);state.message=`Barbed wire recovered for ${WIRE_REFUND} Metal.`;};
 const demolishAt=(point:Vec2)=>{const wall=builtWalls.find(w=>point.x>=w.x&&point.x<w.x+w.width&&point.y>=w.y&&point.y<w.y+w.height);if(wall){removeWall(point);return;}if(builtWires.some(w=>point.x>=w.x&&point.x<w.x+w.width&&point.y>=w.y&&point.y<w.y+w.height)){removeWire(point);return;}state.message='Only player-built Metal Walls and Barbed Wire can be demolished.';};
...
/* Legacy demolition preview implementation.
 ...
 function tick(){
   ...
   if(state.mode==='game'&&run.model.phase==='combat'&&(builtWalls.length||builtWires.length)){
     ...
     if(collapsed.length||breached.length||spent.length){
       ...
       map={...map,obstacles:map.obstacles.filter(obstacle=>!collapsed.includes(obstacle as typeof collapsed[number])&&!breached.includes(obstacle as typeof breached[number])&&!spent.includes(obstacle as typeof spent[number]))};navigation=buildNavigation(map);run.setMap(map);
...
// Legacy demolition rendering implementation.
...
*/
     ...
     const wallGhost=state.mode==='game'&&wallTool&&pointer?{...wallAt(pointer),valid:!defensePlacementIssue(map,run.model.towers,wallAt(pointer),run.model.metal,WALL_COST)}:undefined;
