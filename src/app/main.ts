...
const run=createRun();
const state:UIState={mode:params.get('mode')==='lab'?'lab':'game',phase:'preparation',paused:false,fps:0,frameMs:0,population:10000,capacity:65536,kills:0,crushKills:0,metal:650,baseHealth:100,wave:0,waveCount:5,difficulty:1,selected:null,selectedKind:null,heatmap:false,tool:'blast',message:'Connecting to local GPU…',adapter:'WebGPU',bonusChoices:[],commandUpgrades:[]};
...
try {
 ...
 let builtWalls:Rect[]=[];
 let wireTool=false;
 let builtWires:(Rect & {health:number;maxHealth:number})[]=[];
 ...
 handleAction=action=>{
   ...
   switch(action.type){
     ...
     case 'wire-tool':wireTool=!wireTool;wallTool=false;state.selectedKind=null;state.message=wireTool?'Barbed wire: place a pass-through field that slows and cuts the swarm.':'Barbed wire tool cancelled.';break;
     case 'start-wave':{
 ...
 const placeWire=(wire:Rect)=>{if(builtWires.some(existing=>existing.x===wire.x&&existing.y===wire.y))return;const result=run.spendMetal(45);if(!result.ok){state.message=result.reason??'Could not place wire.';return;}const stats=barbedWireStats(run.model.commandUpgrades),placed={...wire,health:stats.durability,maxHealth:stats.durability};builtWires.push(placed);state.message='Barbed wire installed. Zombies pass through while it cuts and slows them.';};
 const removeWire=(point:Vec2)=>{const index=builtWires.findIndex(w=>point.x>=w.x&&point.x<w.x+w.width&&point.y>=w.y&&point.y<w.y+w.height);if(index<0)return;builtWires.splice(index,1);run.refundMetal(22);state.message='Barbed wire recovered for 22 Metal.';};
 ...
 function tick(){
   ...
   if(state.mode==='game'&&run.model.phase==='combat'&&builtWires.length){const contact=Math.min(1,latest.live/30),erosion=clock.step*wireStats.damage*contact*(1+Math.max(0,latest.maxPacking-1)*.4);const spent=builtWires.filter(wire=>(wire.health-=erosion)<=0);if(spent.length){builtWires=builtWires.filter(wire=>wire.health>0);state.message=`${spent.length} barbed wire section${spent.length===1?'':'s'} wore out after cutting through the swarm.`;}}
 ...
 function frame(now:number){
   ...
   try{
     ...
     renderer.encode(encoder,{count:editor.active?0:count,time:simulatedTime,map:editor.active?editor.map:map,towers:!editor.active&&state.mode==='game'?run.model.towers:[],effects:editor.active?[]:visuals,wires:editor.active?[]:builtWires,heatmap:state.heatmap,selection:run.model.selected,ghost:editor.active?undefined:ghost,wallGhost,boss:!editor.active&&latest.boss?.active?latest.boss:undefined});
