...
/* Recycle hover branch variant is superseded here by the placement-preview wall model. */
 ...
 handleAction=action=>{
   ...
   switch(action.type){
     ...
     case 'wall-tool':wallTool=!wallTool;wireTool=false;state.buildTool=wallTool?'wall':null;state.selectedKind=null;state.message=wallTool?'Wall tool: click to place a 4 × 4 Metal wall. Keep at least one route to the goal.':'Wall tool cancelled.';break;
