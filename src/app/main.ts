 ...
 handleAction=action=>{
   ...
   switch(action.type){
     ...
     case 'restart-wave':{
       ...
       gpu.device.queue.writeBuffer(gpu.shared.counters,0,new Uint32Array(16));physics.reset();combat.reset();boss.reset(run.isBossWave);waveStartTick=clock.tick+1;lastTickSample=clock.tick;state.paused=false;state.selectedKind=null;
       break;
     }
