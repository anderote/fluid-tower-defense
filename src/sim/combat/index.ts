...
@compute @workgroup_size(128) fn hit(@builtin(global_invocation_id) gid:vec3u){
 ...
 for(var t=0u;t<u32(params.clock.w);t++){
  ...
  if(kind==2u){let rail=def.flags.w>.5;if(rail){let forward=vec2f(cos(s.shot.w),sin(s.shot.w));let offset=p.pos.xy-def.position.xy;let along=dot(offset,forward);let across=abs(offset.x*forward.y-offset.y*forward.x);if(along>=0.&&along<=def.position.z&&across<=1.05){p.body.z-=def.weapon.y*max(.35,1.-along/max(def.position.z,.001));}}else if(s.shot.y>=0&&u32(s.shot.y)==i&&s.shot.z==p.status.w){p.body.z-=def.weapon.y;if(def.flags.y==1){p.status.x=max(p.status.x,.25);}}continue;}
