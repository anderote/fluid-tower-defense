...
`;
export const SHAMBLER_ANIMATION_WGSL=`${PARTICLE_WGSL}${SHAMBLER_STATE_WGSL}${ENEMY_WGSL}${ZOMBIE_ROSTER_WGSL}
...
@compute @workgroup_size(128) fn update(@builtin(global_invocation_id) gid:vec3<u32>){
 ...
 let impulse=length(p.pos.zw-a.motion.xy)>max(3.,normalSpeed)+clock.y*40.;
 let hurt=p.body.z<a.motion.z-.01;
 a.pose.z=max(0.,a.pose.z-clock.y);
 if(impulse||hurt){a.pose.z=.22;}
 let stagger=a.pose.z>0.||speed>normalSpeed*1.8;
 if(!stagger&&moving&&speed>.12){
  let heading=atan2(p.pos.w,p.pos.z);let turn=atan2(sin(heading-a.pose.x),cos(heading-a.pose.x));
  let turnLimit=clock.y*zombieTurnRate(kind);a.pose.x+=clamp(turn,-turnLimit,turnLimit);
  a.pose.x=atan2(sin(a.pose.x),cos(a.pose.x));
  a.pose.y=fract(a.pose.y+min(moved,clock.y*normalSpeed*1.23)/zombieStride(kind));
 }
