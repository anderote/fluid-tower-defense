...
fn world(p:vec2<f32>)->vec2<f32>{ return vec2((p.x/160.0)*2.0-1.0, 1.0-(p.y/100.0)*2.0); }
@vertex fn vs(@builtin(vertex_index) vi:u32,@builtin(instance_index) ii:u32)->Out {
  ...
  let hp=clamp(p.body.z/max(0.001,p.body.w),0.0,1.0); let pressure=clamp(p.state.y*.018,0.0,1.0);
  o.color=vec4(mix(col,vec3(1.0,0.18,0.05),pressure)*(0.45+0.55*hp),p.state.w); return o;
}
...
@vertex fn vs(i:I)->O { var o:O; o.pos=vec4((i.pos.x/160.0)*2.-1.,1.-(i.pos.y/100.0)*2.,0,1);o.color=i.color;return o; }
...
@fragment fn fs(@builtin(position) p:vec4<f32>)->@location(0) vec4<f32>{let uv=p.xy/camera.viewport.xy;let w=vec2(uv.x*160.,(1.-uv.y)*100.);let line=(1.-smoothstep(.0,.035,abs(fract(w.x/10.-.5)-.5)))+(1.-smoothstep(.0,.035,abs(fract(w.y/10.-.5)-.5)));let scan=.018*sin(w.x*.7+w.y*.8+camera.time.x*1.5);let heat=exp(-pow((w.x-110.)/46.,2.)-pow((w.y-50.)/34.,2.))*camera.time.y;return vec4(vec3(.035,.055,.045)+line*vec3(.035,.075,.055)+scan+heat*vec3(.12,.045,0),1);}`});
  ...
  return { encode(encoder,scene){resize(); const u=new Float32Array([pixelW,pixelH,0,0,0,0,W,H,scene.time,scene.heatmap?1:0,0,0,0,0,0,0]);device.queue.writeBuffer(uniform,0,u);const data=geometry(scene);if(data.byteLength)device.queue.writeBuffer(overlays,0,data.buffer,data.byteOffset,data.byteLength);const pass=encoder.beginRenderPass({colorAttachments:[{view:context.getCurrentTexture().createView(),clearValue:{r:.02,g:.03,b:.025,a:1},loadOp:'clear',storeOp:'store'}]});pass.setPipeline(bg);pass.setBindGroup(0,cameraBG);pass.draw(3);pass.setPipeline(overlay);pass.setBindGroup(0,cameraOverlay);pass.setVertexBuffer(0,overlays);pass.draw(Math.min(data.length/6,MAX_OVERLAY_VERTICES));pass.setPipeline(particles);pass.setBindGroup(0,cameraParticles);pass.draw(6,Math.min(scene.count,shared.capacity));pass.end(); },
