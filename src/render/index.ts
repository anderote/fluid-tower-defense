...
@fragment fn fs(@builtin(position) p:vec4<f32>)->@location(0) vec4<f32>{let keepBinding=camera.viewport.x*0.;return vec4(vec3(.075+keepBinding),1.);}`});
  ...
  const wireShape=(a:V[],wire:{x:number;y:number;width:number;height:number},c:[number,number,number,number],integrity=1)=>{const post:[number,number,number,number]=[.32,.36,.39,c[3]];rect(a,wire.x+.28,wire.y+.25,.2,wire.height-.5,post);rect(a,wire.x+wire.width-.48,wire.y+.25,.2,wire.height-.5,post);for(let row=0;row<3;row++){const y=wire.y+.85+row*1.12;streak(a,wire.x+wire.width-.42,y+.44,1,.16,wire.width-.78,.055,c);streak(a,wire.x+wire.width-.42,y-.44,1,-.16,wire.width-.78,.055,c);}for(let x=wire.x+.75;x<wire.x+wire.width-.35;x+=.72){streak(a,x,wire.y+1.1,1,-1,.42,.055,[Math.min(1,c[0]+.14),Math.min(1,c[1]+.18),c[2],c[3]*integrity]);streak(a,x,wire.y+3.05,1,1,.42,.055,[Math.min(1,c[0]+.14),Math.min(1,c[1]+.18),c[2],c[3]*integrity]);}};
