...
function author(biome:Biome,title:string,briefing:string){
  ...
  const road=(x:number,y:number,length:number)=>{for(let xx=x;xx<x+length;xx+=16)stamp('d10',xx,y);};
  ...
  return {map,scenery,solid,stamp,prop,wall,road,grove,ridge};
}
...
function forest(){
  ...
  // Tracks pass the village, with small rocky clearings behind the cliff line.
  a.road(0,48,80);a.road(76,20,32);a.road(76,76,32);a.road(112,48,48);
  for(let y=28;y<76;y+=8){a.stamp('d03',76,y);a.stamp('d03',108,y);}
