...
for(const level of [1,2,3]){
  test(`campaign ${level}: authored bounds, entry frontage, scenery assets and boss route are valid`,()=>{
    ...
    for(const tile of scenery.tiles){assert.ok(atlas.sprites[tile.sprite]?.length>=tile.columns*tile.rows,tile.sprite);assert.ok(tile.x>=0&&tile.y>=0&&tile.x+tile.columns*4<=map.width&&tile.y+tile.rows*4<=map.height,tile.sprite+' bounds');}
