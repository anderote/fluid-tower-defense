...
export function damMap():WorldMap{
 ...
 return {id:DAM_ID,width:160,height:100,spawn:{x:0,y:20,width:8,height:60},goal:{x:154,y:50},goalRadius:4,obstacles:platforms.map(r=>({...r})),dam:freshDam(),scenery:{biome:'interior',title:'Thunderhead Dam',briefing:'Flood all three spillways. Close the north and south gates to divert enemies; the central spillway stays open. Mount towers on the concrete islands.',solids:[],mounts:platforms.map(r=>({...r})),tiles:[],props:[],regions:[]}};
}
