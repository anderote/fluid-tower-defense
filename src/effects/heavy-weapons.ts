...
export function createHeavyProjectiles(kind:HeavyProjectile['kind'], origin:Vec2, target:Vec2, serial:number,peakPressureKpa=kind==='rocket'?1250:650):HeavyProjectile[]{
  ...
  if(kind==='mortar')return [{...muzzle,kind,target:{...target},age:0,delay:0,flight:.54,serial,lane:0,peakPressureKpa}];
  return [-1,0,1].map((lane,index)=>({...muzzle,kind,target:{x:target.x+side.x*lane*2.1,y:target.y+side.y*lane*2.1},age:0,delay:index*.045,flight:.5,serial:serial*3+index,lane,peakPressureKpa}));
}
