...
export interface DamState {closed:[boolean,boolean];reservoir:number;surge:number;switchCooldown:number}
export interface WorldMap { dam?:DamState; id: string; width: number; height: number; obstacles: Rect[]; spawn: Rect; goal: Vec2; goalRadius: number; scenery?:MapScenery }
export type TowerKind = 'repulsor' | 'mortar' | 'autocannon' | 'cryo' | 'tesla' | 'rocket' | 'railgun' | 'incinerator' | 'crusher';
export type EnemyKind = 'shambler' | 'runner' | 'brute' | 'rager' | 'softbody' | 'husk';
export type EffectKind = 'blast' | 'push' | 'slow' | 'shot' | 'crush' | 'flood';
...
export interface Tower extends Vec2 { id: number; kind: TowerKind; level: number; branch: number; angle: number; cooldown: number; spent: number; crusherAnimation?:number; crusherRecharge?:number; kills?:number; veterancy?:number; veterancyXp?:number }
