...
export type TowerKind = 'repulsor' | 'mortar' | 'autocannon' | 'cryo';
...
export interface Tower extends Vec2 { id: number; kind: TowerKind; level: number; branch: number; angle: number; cooldown: number; spent: number }
...
export interface RenderScene { count: number; time: number; map: WorldMap; towers: readonly Tower[]; effects: readonly Effect[]; heatmap: boolean; selection: number | null; ghost?: Vec2 & {kind: TowerKind; valid: boolean; range:number}; boss?: Vec2 & {health:number;maxHealth:number;phase:number} }
...
export type GameAction = {type:'mode';mode:'lab'|'game'} | {type:'pause'} | {type:'step'} | {type:'reset'} | {type:'start-wave'} | {type:'select-tower';kind:TowerKind|null} | {type:'upgrade';branch:number} | {type:'sell'} | {type:'heatmap';value:boolean} | {type:'population';value:number} | {type:'tool';tool:'blast'|'push'|'inspect'} | {type:'bonus';id:string} | {type:'save'} | {type:'load'};
export interface BonusChoice { id:string; name:string; description:string }
export interface UIState { mode:'lab'|'game'; phase:'preparation'|'combat'|'settling'|'won'|'lost'; paused:boolean; fps:number; frameMs:number; population:number; capacity:number; kills:number; crushKills:number; scrap:number; baseHealth:number; wave:number; waveCount:number; selected:Tower|null; selectedKind:TowerKind|null; heatmap:boolean; tool:'blast'|'push'|'inspect'; message:string; adapter:string; bonusChoices:readonly BonusChoice[]; bossHealth?:number; }
...
export interface RunModel { phase:UIState['phase']; scrap:number; baseHealth:number; wave:number; waveCount:number; towers:Tower[]; selected:number|null; pending:SpawnBatch[]; bonusChoices:BonusChoice[]; bonuses:string[] }
