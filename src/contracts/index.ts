...
export type GameAction = {type:'mode';mode:'lab'|'game'} | {type:'pause'} | {type:'restart-wave'} | {type:'reset'} | {type:'new-game'} | {type:'start-wave'} | {type:'continue-run'} | {type:'finish-run'} | {type:'select-tower';kind:TowerKind|null} | {type:'unlock-tower';kind:TowerKind} | {type:'wall-tool'} | {type:'wire-tool'} | {type:'demolish-tool'} | {type:'upgrade-tool'} | {type:'upgrade';branch:number} | {type:'buy-command';id:string} | {type:'buy-stat';id:string} | {type:'sell'} | {type:'difficulty';value:number} | {type:'stream-width';value:number} | {type:'heatmap';value:boolean} | {type:'population';value:number} | {type:'tool';tool:'blast'|'push'|'inspect'} | {type:'bonus';id:string} | {type:'save'} | {type:'load'};
...
export interface UIState { mode:'lab'|'game'; phase:'preparation'|'combat'|'settling'|'checkpoint'|'won'|'lost'; paused:boolean; fps:number; frameMs:number; population:number; capacity:number; kills:number; crushKills:number; leaks:number; earned:number; maxPressure:number; metal:number; baseHealth:number; level:number; wave:number; waveCount:number; difficulty:number; streamWidth:number; selected:Tower|null; selectedKind:TowerKind|null; buildTool:'wall'|'wire'|'demolish'|null; upgradeMode:boolean; heatmap:boolean; tool:'blast'|'push'|'inspect'; message:string; adapter:string; bonusChoices:readonly BonusChoice[]; bonuses:readonly string[]; commandUpgrades:readonly string[]; statUpgrades:readonly StatUpgrade[]; towerUnlocks:readonly TowerUnlock[]; boss?:Settlement['boss']; bossHealth?:number; }
