...
export type GameAction = {type:'mode';mode:'lab'|'game'} | {type:'pause'} | {type:'restart-wave'} | {type:'reset'} | {type:'start-wave'} | {type:'select-tower';kind:TowerKind|null} | {type:'wall-tool'} | {type:'wire-tool'} | {type:'upgrade';branch:number} | {type:'buy-command';id:string} | {type:'buy-meta';id:string} | {type:'sell'} | {type:'difficulty';value:number} | {type:'heatmap';value:boolean} | {type:'population';value:number} | {type:'tool';tool:'blast'|'push'|'inspect'} | {type:'bonus';id:string} | {type:'save'} | {type:'load'};
